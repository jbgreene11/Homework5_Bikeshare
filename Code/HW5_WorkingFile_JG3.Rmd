---
title: "MUSA 508 - Homework 5"
author: "Jay Greene (Partner: Esther Yihan Zhang, Brian Rawn)"
date: "11/15/2020"
...
load('C:/Users/green/OneDrive/Documents/00_Fall2020/00_MUSA508/1113.rdata')
```

## 1.1 Introduction
  
  The following study was conducted to operationalize bike ride-sharing stations in Boston, Massachusetts. Boston has a city-wide goal of increasing bike usage by 400% by 2030. Towards that effort, the city has put substantial funding into infrastructure development of safer bike routes. The city has added an additional 50 new bikes share stations since 2019 and has held over 42 public meetings to determine where to add more. This study represents the kind of information that is necesary to facilitate public-private partnerships. Bluebikes is owned by the municipalities of Boston, Brookline, Cambridge, Somerville and Everet, but operated by a private company called Motivate. Analysis into this operation, therefore, would require a combination of publically availble data bout the spatial conditions of the bikeshare program, and data collected by the company on users. Becuase the program is publicly owned, Motivate publishes real-time data about the program's use.

The purpose of this study was to predict the demand of bikes among boston-based bike stations. In addition to learning to use time-space predictive modeling to address operational challenges, we learned how nuanced insights into the capacity of predictive models can be used for business strategy. 

This model can be used to develop both a pro-active bike operations strategy and re-active strategy. Proactively, were we have accurately predicted the demand of bikes, we can develop a resource-intensive strategy for meeting this demand. In practice, this could be trucks with pre-determined routes and staffing. For the times when we seem to be consistently under-predicting, wecould develop a more re-active stragey like surge-pricing to intice customers to redistribute the bikes as needed. Either way, predictive models help us identify both constants and unknowns in the process. 


## 2.1 Setup

First, we will load relevant libraries and graphic themes. 

```{r, catche = TRUE, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(sf)
library(lubridate)
library(tigris)
...
```{r, eval=FALSE, catche = TRUE, echo=FALSE, message=FALSE, warning=FALSE}
plotTheme <- theme(
...
mapTheme1 <- function(base_size = 12) {
  theme(
...
plotTheme1 <- function(base_size = 12) {
  theme(
    text = element_text( color = "black"),
    plot.title = element_text(size = 16,colour = "black"),
    plot.subtitle = element_text(face="italic"),
...
The bikeshare data was harvested by the bluebikes program and make public through the Analyze Boston dataplatform. We analyzed bikeshare data for April 2018. 
...
Census geography and varaibles were collected using tidycensus package. Then, spatial information of rideshare data was joined to station origin. 
...
Weather data from Boston International Airport (KBOS) was collected using reim_measures. The plots display percipitation, wind speed, adn temprarture on an hourly basis over the study period. 
...
Exploration of the dataset starts with time and frequency components. there appears to be significantly more usage during weekdays. Additionally, there appears to be a week of lower usage in the middle of the month. This correlates with public school's spring break period for 2018. 
...
Next we examined the  distribution of trip volume by station for different times of day. There appears to be significantly higher usage during the AM and PM rush. 

```{r, echo=FALSE, warning=FALSE, cache=TRUE, warning=FALSE, results=FALSE}
dat_census %>%
...
```
The following plots display bike share trips by day of the week and on weekend veruss weekday. Weekday patterns seem to follow similar patterns, with a spike in usage udring the AM and PM rush. Weekend appears to be a smoother distribution, with peak hours in the early afternoon. Overall usage is significantly lower on the weeekends. 
...
```
The plot below displays bike share trips per hour by station.The maps confirm the analysis above - weekday AM and PM rushes are seen to be concentrated in the downtown commercial districts. Weekend use is more evenly distrubted throughout the day, and throughout residential areas.  
...
We then make sure each unique station and hour/day combo exists in our dataset. we created a panel data set where each time period in the study is represented by a row.
...
We create the full panel which summarizes counts by station for each time internal and remove station IDs that are FALSE. 
...
This animated map shows usage overtime. heavy concentration of rides are seen to occur during AM and PM rushes in the downtown commercial district. 
...
Creating time lag variables. We controled for the effects of the Easter Holiday (April 1st) and for dummy lag variables indicated temporal proximity to the holiday. 
...
ride.panel <- 
  ...
  mutate(lagHour = dplyr::lag(Trip_Count,1),
   ...
   mutate(holidayLag = case_when(dplyr::lag(holiday, 1) == 1 ~ "PlusOneDay",
         ...
         holidayLag = replace_na(holidayLag))
...
A feature was created to account for weekend versus weekday use. 
...
Input Nearest neighbor function for analyis of neighborhood spatial characteristics. 
```{r, eval=FALSE, catche = TRUE, echo=FALSE, message=FALSE, warning=FALSE}
nn_function <- function(measureFrom,measureTo,k) {
  measureFrom_Matrix <- as.matrix(measureFrom)
  measureTo_Matrix <- as.matrix(measureTo)
...
Create feature for distance to water and parkways. 
...
Create feature to see See if the destination is within the 100m buffer of parks larger than 15 acres.
...
Create categorical variable for distance to colleges and universities. 
...
We then split our data into a training and a test set set and train the model on the data from the first two weeks of april. We split the data by time and not randomly becuase of the temporal relationship of demand. 
...
After the models have fun, we create a nested data frame to test data by week. 
...
The model that includes all of the added  amenity spatial features functions better than the other models. Holiday lag times do not seem to have an effect. 
...
Plot Predicted/Observed Bike share time series. We seem to be consistently under predicting the data in all models. 
...
Plot errors on a map by weekend/weekday and time of day. There appears to be some spatial clustering of errors. Based on the location of the errors, they are occuring in an area with a high number of hospitals. Perhaps we have not adequately accounted for the higher demand in statations near hospitals. 
...
Focus on the morning commute - Plot Errors as a function of socio-economic variables. We seem to be predicting evenly across income and percent transit users, but appear to be under predctiing in areas with higher percentage white residents. 
...
This study has begun to predict the demand of bikeshare bikes in Boston, MA. Through temporal & spatial analysis, we have demonstrated that demand in certain stations can be identified and predicted. While the model did not create the accuracy needed for business use, the study demonstrates the type of spatial and temporal variables that impact the demand of bikes. Distance to metro stations and hospitals appear to be significant facotrs in determining the demand of bikes. Temporal factors such as day of the week are significant determminants of bike usage. Additional study would be needed to determine if the clusering observed are purely spatial reasons or if there are demographic considerations. For example - does the high demand for bikes near hospitals occur becasue there is a lack of available parking, or because of the demographics of the populations who work there? Analyis of this sort introduces the concepts of spatial and temporal intelligence in addition to operationalizing deicisons in public-private partnerships. 
